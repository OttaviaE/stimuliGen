...
DrawRegPolygon(x = circle()$pos.x, y = circle()$pos.y, radius.x=circle()$size.x,
               ...
               lwd = circle()$lwd, lty=circle()$lty, nv = 100)
```
...
```{r}
cof(campo_generico,circle(),triangle())
...
-  `obj` il contenuti di una cella della matrice
...
# Top Left Low Right
M<-apply(Raven(st1=square(),hrule=c("rotation"),vrule=c("rotation")))
draw(M)
