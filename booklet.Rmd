...
# vertical canvas
draw(vline(pos.x=-30, s.x = 30, lwd = 3),bg = col)
for(i in seq(-30, 30, by = 1)) {
  draw(vline(pos.x = i, s.x=40, lwd = 3), 
       canvas = F)
}
...
```{r}
draw(hline(pos.y=-17, s.x = 30), bg = col)
for(i in seq(-17, 17, by = 1)) {
  draw(hline(pos.y = i, s.x=30, lwd = 3), 
       canvas = F)
}
...
```{r}
draw(rectangle(s.x=7,s.y=5,shd="line.12"),
     canvas = T)

for(i in seq(-7, 7, by = 1.5)) {
  draw(diagline(pos.x = i, s.x=-7, lwd = 3), 
       canvas = F)
}
...
```{r}
draw(diagline(pos.x=-50, s.x = 50, lwd = 3), bg = col)
for(i in seq(-50, 50, by = 3.5)) {
  draw(diagline(pos.x = i, s.x=40, lwd = 3), 
       canvas = F)
}
...
## Non riesco a fare i distrattori perché non mi riconosce line12inv come colore 
...
Odio massimiliano dal profondo del mio cuore
